...
static GdkPixbuf*
_gdk_pixbuf_new_from_inline (gint          data_length,
			    const guint8 *data,
			    gboolean      copy_pixels,
			    GError      **error)
{
  ...
  if (!_gdk_pixdata_deserialize (&pixdata, data_length, data, error))
    return NULL;
  return _gdk_pixbuf_from_pixdata (&pixdata, copy_pixels, error);
}
		     ...
		     GtkDragSourceInfo *info)
{
  ...
  if (drag_cursors[i].pixbuf == NULL)
    drag_cursors[i].pixbuf = 
      _gdk_pixbuf_new_from_inline (-1, drag_cursors[i].data, FALSE, NULL);
