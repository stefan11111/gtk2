	    ...
	    GError             **error)
{
  if (info->create_tags)
	...
	return g_hash_table_lookup (info->anonymous_tags,
      ...
      const gchar *real_name = g_hash_table_lookup (info->substitutions, name);
                         ...
                         GError **error)
{
  ...
  return  _gdk_pixbuf_from_pixdata (&pixdata, TRUE, error);
}
			 ...
			 GError              **error)
{
  ...
  if (ELEMENT_IS ("apply_tag"))
	...
	return;
      ...
      GtkTextTag *tag = tag_exists (context, name, id, info, error);
